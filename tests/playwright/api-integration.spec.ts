...
test.describe('API Integration Tests', () => {
  ...
  test('should handle scan API response structure', async ({ page }) => {
    ...
    const scanResponse = await page.request.post('http://localhost:8081/api/scan/microsoft?max_repos=3', {
      timeout: 60000,
    });
    ...
    if (scanResponse.status() === 201) {
      ...
      console.log(`Scan successful: ${scanData.data.summary.totalRepositories} repos, ${scanData.data.summary.totalCodeowners} codeowners`);
    } else {
  ...
  test('should validate graph API response against Zod schemas', async ({ page }) => {
    // First scan an organization (optional - might already be scanned)
    await page.request.post('http://localhost:8081/api/scan/facebook?max_repos=2', {
      timeout: 30000,
    }).catch(() => {
    ...
    // Test graph API
    const graphResponse = await page.request.get('http://localhost:8081/api/graph/facebook');
    ...
    if (graphResponse.status() === 200) {
      ...
      console.log(`Graph data validated: ${graphData.data.nodes.length} nodes, ${graphData.data.edges.length} edges`);
    } else {
  ...
  test('should handle useTopics parameter correctly', async ({ page }) => {
    // Test without topics
    const teamsResponse = await page.request.get('http://localhost:8081/api/graph/github');
    
    if (teamsResponse.status() === 200) {
      const teamsData = await teamsResponse.json();
      
      // Test with topics
      const topicsResponse = await page.request.get('http://localhost:8081/api/graph/github?useTopics=true');
      ...
      if (topicsResponse.status() === 200) {
        ...
        // Log the difference
        const teamNodeTypes = new Set(teamsData.data.nodes.map((n: any) => n.type));
        const topicNodeTypes = new Set(topicsData.data.nodes.map((n: any) => n.type));
        ...
        if (topicsData.data.nodes.length > 0) {
          const hasTopicNodes = topicsData.data.nodes.some((n: any) => n.type === 'topic');
          if (hasTopicNodes) {
            console.log('✅ Topics parameter is working - found topic nodes');
          } else {
            console.log('ℹ️  No topic nodes found - organization might not have topics');
          }
        }
      }
    }
  ...
  test('should handle frontend-backend integration flow', async ({ page }) => {
    ...
    const hasCanvas = await page.locator('[data-testid="graph-canvas"]').isVisible();
    ...
    if (hasCanvas) {
      ...
      console.log('ℹ️  Graph failed to load - this might be expected if backend is not running or org has no data');
  ...
  test('should handle API timeout gracefully', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Enter organization name"]', 'timeout-test');
    await page.click('button:has-text("Load Graph")');
  ...
  test('should validate node type consistency', async ({ page }) => {
    ...
    const graphResponse = await page.request.get('http://localhost:8081/api/graph/github');
    
    if (graphResponse.status() === 200) {
      const graphData = await graphResponse.json();
      
      const validNodeTypes = ['organization', 'repository', 'user', 'team', 'topic'];
      const foundNodeTypes = new Set(graphData.data.nodes.map((n: any) => n.type));
