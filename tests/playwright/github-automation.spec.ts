...
test.describe('GitHub Automation', () => {
  test('should scan organization first, then load graph with topics', async ({
    page,
  }) => {
    // Navigate to the app
    await page.goto('/')
    ...
    const scanResponse = await page.request.post(
      'http://localhost:8081/api/scan/github?useTopics=true&max_repos=5',
      {
    ...
    const graphResponse = await page.waitForResponse(
      (response) =>
        response.url().includes('localhost:8081/api/graph/github') &&
        response.status() === 200,
    ...
    if (nodes.length === 0 && edges.length === 0) {
      console.log(
        '⚠️  WARNING: Graph data is empty - this may indicate the useTopics parameter is not being processed correctly'
      )
    } else {
      console.log(
        `✅ SUCCESS: Graph loaded with ${nodes.length} nodes and ${edges.length} edges`
      )
    }
  })

  test('should demonstrate the proper API workflow with topics parameter', async ({
    page,
  }) => {
  ...
  test('should handle errors gracefully when organization does not exist', async ({
    page,
  }) => {
    ...
    const scanResponse = await page.request.post(
      `http://localhost:8081/api/scan/${nonExistentOrg}?useTopics=true`,
      {
        timeout: 30000,
      }
    )
