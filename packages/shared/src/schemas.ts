...
export const HealthResponseSchema = z.object({
  data: z.object({
    ...
    timestamp: z.string().datetime().describe('Current timestamp in ISO 8601 format'),
  }),
});
...
export const ScanSummarySchema = z.object({
  total_repos: z.number().int().min(0).describe('Total number of repositories scanned'),
  repos_with_codeowners: z.number().int().min(0).describe('Number of repositories with CODEOWNERS files'),
  total_teams: z.number().int().min(0).describe('Total number of teams found'),
  unique_owners: z.array(z.string()).describe('List of unique codeowners (users and teams)'),
  api_calls_used: z.number().int().min(0).describe('Number of GitHub API calls used'),
  processing_time_ms: z.number().int().min(0).describe('Processing time in milliseconds'),
});
...
export const ScanResponseSchema = z.object({
  data: z.object({
    ...
    errors: z.array(z.string()).describe('List of errors encountered during scanning'),
    data: z.object({}).passthrough().optional().describe('Raw scan data including organization, repositories, teams, and codeowners'),
  }),
});
 ...
 */
export const GraphNodeTypeSchema = z.enum(['organization', 'repository', 'team', 'user']);
...
export const StatsResponseSchema = z.object({
  data: z.object({
    ...
    total_repositories: z.number().int().min(0).describe('Total number of repositories'),
    ...
    total_codeowners: z.number().int().min(0).describe('Total number of codeowners entries'),
    codeowner_coverage: z.string().describe('Percentage of repositories with codeowners'),
    last_scan_time: z.string().datetime().describe('Timestamp of last scan in ISO 8601 format'),
  }),
});
...
export const ScanRequestParamsSchema = z.object({
  ...
  max_repos: z.number().int().min(1).max(1000).default(100).optional().describe('Maximum number of repositories to scan'),
  max_teams: z.number().int().min(1).max(500).default(50).optional().describe('Maximum number of teams to scan'),
});
...
export const GraphRequestParamsSchema = z.object({
  org: z.string().min(1).describe('GitHub organization name'),
  useTopics: z.boolean().default(false).optional().describe('Use repository topics instead of teams for graph visualization'),
});
...
// Validation Error class using Effect.ts Data.TaggedError
  ...
  Effect.try({
    try: () => schema.parse(data),
      ...
      if (error instanceof z.ZodError) {
        ...
        return new ValidationError({ message, context, zodError: error });
      }
...
// Synchronous version for backwards compatibility (throws exceptions)
