...
// buildDeleteOrganizationDataQuery builds a query to delete organization data (Pure Core)
func buildDeleteOrganizationDataQuery() string {
	return `
		MATCH (org:Organization {login: $org_login})
		OPTIONAL MATCH (org)-[:OWNS]->(repo:Repository)
		OPTIONAL MATCH (org)-[:HAS_TEAM]->(team:Team)
		OPTIONAL MATCH (repo)-[:HAS_CODEOWNER]->(user:User)
		OPTIONAL MATCH (repo)-[:HAS_TEAM_OWNER]->(team_owner:Team)
		DETACH DELETE org, repo, team, user, team_owner
	`
}
...
func convertToGraphNodes(records []map[string]interface{}) []GraphNode {
	...
	if orgNode, exists := record["org_node"]; exists {
		if orgMap, ok := orgNode.(map[string]interface{}); ok {
			nodes = append(nodes, convertMapToGraphNode(orgMap, 0, 0))
		}
	}

	// Add repository nodes
	if repos, exists := record["repos"]; exists {
		if repoList, ok := repos.([]interface{}); ok {
			for i, repo := range repoList {
				if repoMap, ok := repo.(map[string]interface{}); ok {
					nodes = append(nodes, convertMapToGraphNode(repoMap, float64(i*200), 200))
				}
			}
		}
	}
	...
	// Add team nodes
	if teams, exists := record["teams"]; exists {
		if teamList, ok := teams.([]interface{}); ok {
			for i, team := range teamList {
				if teamMap, ok := team.(map[string]interface{}); ok {
					nodes = append(nodes, convertMapToGraphNode(teamMap, float64(i*200), 400))
				}
			}
		}
	}
	...
	// Add user nodes
	if users, exists := record["users"]; exists {
		if userList, ok := users.([]interface{}); ok {
			for i, user := range userList {
				if userMap, ok := user.(map[string]interface{}); ok {
					nodes = append(nodes, convertMapToGraphNode(userMap, float64(i*200), 600))
				}
			}
		}
	}
...
func convertToGraphEdges(records []map[string]interface{}) []GraphEdge {
	...
	if edgeList, exists := record["edges"]; exists {
		if edges_array, ok := edgeList.([]interface{}); ok {
			for _, edge := range edges_array {
